...
@Component({
  ...
  styleUrls: ['./notice-generate.component.scss'],
})
export class NoticeGenerateComponent implements OnInit {
  ...
  form = new FormGroup({
    ...
    noticeOfMotion: new FormControl(true, Validators.required),
  ...
  ngOnInit() {
    ...
    const cached = this.cacheService.rehydrate<FormGroup>(
      'NOTICE_MOTION_GENERATION'
    );
  ...
  onSelectSponsor() {
    ...
    const urlTree = this._motionId
      ? ['/generate/notice-of-motion', this._motionId]
  ...
  onSave(published: boolean) {
    ...
    const post = (state: 'public' | 'private' | 'draft') => {
      const navigate = () => {
        this.cacheService.clearCache('NOTICE_MOTION_GENERATION');
