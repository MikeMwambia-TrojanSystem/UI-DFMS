...
export class PetitionService {
  ...
  getPetitions(): Observable<Petition[]> {
    return this._petitions.pipe(
      switchMap((petitions) =>
        iif(() => this._fetched, of(petitions), this.fetchPetitions())
      )
    );
  }
  ...
  getPetition(id: string): Observable<Petition> {
    return this._petitions.pipe(
      ...
      map((petitions) => petitions.find((p) => p._id === id))
    );
  }

  fetchPetitions(): Observable<Petition[]> {
    return this.apiService.getPetitions().pipe(
      tap(({ message }) => {
  ...
  postPetition(petition: PetitionPost) {
    return this.apiService.createPetition(petition).pipe(
      tap(({ message }) => {
        if (this._fetched) {
  ...
  deletePetition(id: string) {
    return this.apiService.deletePetition(id).pipe(
      tap((result) => {
  ...
  updatePetition(petition: PetitionPost) {
    return this.apiService.updatePetition(petition).pipe(
      tap((result) => {
        const newPetitions = this._petitions.getValue();
        const index = newPetitions.findIndex((p) => p._id === result._id);
  ...
  approvePetition(id: string) {
    return this.apiService.approve('petition', id);
  }
}
